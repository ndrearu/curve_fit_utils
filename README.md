...
A somehow more detailed description of the modules (both about the implementation and the statistical part) can be found in the documentation which I recommend to read. However, firstly I suggest to go directly to the examples below and then only later to read all the (probably boring) details.

### Documentation
Details about the why, how and what about the routines in `curve_fit_utils` can be found in the file
[DOCUMENTATION.md](DOCUMENTATION.md). I highly recommend to read the description of the different functions, both about their implementation or especially the statistical stuff. For a first quick use, instead, it is possible directly to look at the module itself [curve_fit_utils.py](curve_fit_utils.py) where a description of the arguments needed, returns and options is reported in the definition of each routine.
